...
void QuadCopterShiftAvoidance::avoid(const std::vector<Obstacle> &obstacles)
{
    ...
    case avoid_state::moving: {
        ...
        Obstacle closest;
        closest.spherical_pose.x = defaults::safe_distance + 1;
        for (Obstacle obs : obstacles) {
            if (obs.spherical_pose.x < closest.spherical_pose.x) {
                closest = obs;
            }
        }

        if (closest.spherical_pose.x > defaults::safe_distance) {
            break;
        }
        ...
        if (mavlink_vehicles::math::ground_dist(
                ...
                vehicle->mav->get_mission_waypoint()) <= closest.spherical_pose.x) {
            break;
        }
