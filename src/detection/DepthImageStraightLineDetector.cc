...
{
    auto buffer = this->sensor->get_depth_buffer();
    // this->obstacles.resize(buffer.size() / (DS_FACTOR * DS_FACTOR));
    this->obstacles.resize(buffer.size() / DS_FACTOR_2);
    auto width = this->sensor->get_width();
    auto height = this->sensor->get_height();
    ...
    for (uint i = 0; i < height; i++) {
        for (uint j = 0; j < width; j++) {
            ...
            double depth = buffer[index] * this->sensor->get_scale();
